...
- Optional siren playback through a media player with curated MP3 selection from Home Assistant's `/media` and `/config/www` folders (no manual typing required).
...
- Optionale Sirene über einen Medienplayer; alle vorhandenen MP3-Dateien aus `/media` und `/config/www` werden automatisch gelistet (kein manuelles Tippen).
